...
int main(void) {
    ...
    std::string texturePath = object_id == 0 ?
        "textures/basketball/NBA_BASKETBALL_DIFFUSE.jpg" :
        "textures/vase/Vase-obj_0.jpg";
    ...
    for (int z = 0; z < total_frame && !glfwWindowShouldClose(window); z++) {
        ...
        if (object_id == 0) {
            myScalingMatrix = glm::scale(mat4(1.0f), vec3(10.0f, 10.0f, 10.0f));
        }
        else {
            myScalingMatrix = glm::scale(mat4(1.0f), vec3(0.9f, 0.9f, 0.9f));
        }
        ...
        ourShader.setMat4("model", BackgroundModel);
